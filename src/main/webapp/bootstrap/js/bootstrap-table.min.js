...
!function (a) {
    ...
    h.DEFAULTS = {classes: "table table-hover", height: void 0, undefinedText: "-", sortName: void 0, sortOrder: "asc", striped: !1, columns: [], data: [], method: "get", url: void 0, cache: !0, contentType: "application/json", dataType: "json", ajaxOptions: {}, queryParams: function (a) {
        return a
        ...
        return b("显示第 %s 到第 %s 条记录，总共 %s 条记录", a, c, d)
        ...
        this.data = "append" === b ? this.data.concat(a) : "prepend" === b ? a.concat(this.data) : a || this.options.data, this.options.data = this.data, "server" !== this.options.sidePagination && this.initSort()
        ...
        -1 !== e && this.data.sort(function (g, h) {
            ...
            return void 0 !== k ? d * k : (a.isNumeric(i) && (i = parseFloat(i)), a.isNumeric(j) && (j = parseFloat(j)), (void 0 === i || null === i) && (i = ""), (void 0 === j || null === j) && (j = ""), a.isNumeric(i) && a.isNumeric(j) ? j > i ? -1 * d : d : i === j ? 0 : -1 === i.localeCompare(j) ? -1 * d : d)
        })
        ...
        var o = ['<span class="btn-group dropup">', '<button type="button" class="btn btn-default ' + (void 0 == this.options.iconSize ? "" : " btn-" + this.options.iconSize) + ' dropdown-toggle" data-toggle="dropdown">', '<span class="page-size">', this.options.pageSize, "</span>", ' <span class="caret"></span>', "</button>", '<ul class="dropdown-menu" role="menu">'], p = this.options.pageList;
    ...
    if ("string" == typeof this.options.pageList) {
        ...
        for (var k = this.pageFrom - 1; k < this.pageTo; k++) {
            var l = j[k], m = {}, n = [], o = {}, p = [];
            if (m = f(this.options, this.options.rowStyle, [l, k], m), m && m.css)for (var q in m.css)n.push(q + ": " + m.css[q]);
            if (o = f(this.options, this.options.rowAttributes, [l, k], o))for (var q in o)p.push(b('%s="%s"', q, g(o[q])));
            i.push("<tr", b(" %s", p.join(" ")), b(' id="%s"', a.isArray(l) ? void 0 : l._id), b(' class="%s"', m.classes || (a.isArray(l) ? void 0 : l._class)), b(' data-index="%s"', k), ">"), this.options.cardView && i.push(b('<td colspan="%s">', this.header.fields.length)), a.each(this.header.fields, function (a, e) {
                var g = "", j = l[e], o = "", p = {}, q = "", r = h.header.classes[a], s = h.options.columns[d(h.options.columns, e)];
                if (m = b('style="%s"', n.concat(h.header.styles[a]).join("; ")), j = f(h.header, h.header.formatters[a], [j, l, k], j), l["_" + e + "_id"] && (q = b(' id="%s"', l["_" + e + "_id"])), l["_" + e + "_class"] && (r = b(' class="%s"', l["_" + e + "_class"])), p = f(h.header, h.header.cellStyles[a], [j, l, k], p), p.classes && (r = b(' class="%s"', p.classes)), p.css) {
                    var t = [];
                    for (var u in p.css)t.push(u + ": " + p.css[u]);
                    m = b('style="%s"', t.concat(h.header.styles[a]).join("; "))
                }
                if (s.checkbox || s.radio) {
                    if (h.options.cardView)return!0;
                    o = s.checkbox ? "checkbox" : o, o = s.radio ? "radio" : o, g = ['<td class="bs-checkbox">', "<input" + b(' data-index="%s"', k) + b(' name="%s"', h.options.selectItemName) + b(' type="%s"', o) + b(' value="%s"', l[h.options.idField]) + b(' checked="%s"', j === !0 || j && j.checked ? "checked" : void 0) + b(' disabled="%s"', !s.checkboxEnabled || j && j.disabled ? "disabled" : void 0) + " />", "</td>"].join("")
                } else j = "undefined" == typeof j || null === j ? h.options.undefinedText : j, g = h.options.cardView ? ['<div class="card-view">', h.options.showHeader ? b('<span class="title" %s>%s</span>', m, c(h.options.columns, "field", "title", e)) : "", b('<span class="value">%s</span>', j), "</div>"].join("") : [b("<td%s %s %s>", q, r, m), j, "</td>"].join(""), h.options.cardView && h.options.smartDisplay && "" === j && (g = "");
                i.push(g)
        ...
        "server" === this.options.sidePagination && (this.options.totalRows = a.total, a = a.rows), this.initData(a), this.initSearch(), this.initPagination(), this.initBody()
    ...
    var i = ["getOptions", "getSelections", "getData", "load", "append", "prepend", "remove", "insertRow", "updateRow", "mergeCells", "checkAll", "uncheckAll", "check", "uncheck", "refresh", "resetView", "destroy", "showLoading", "hideLoading", "showColumn", "hideColumn", "filterBy", "scrollTo", "selectPage", "prevPage", "nextPage", "togglePagination", "toggleView"];
